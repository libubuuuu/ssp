...
export default function VideoCloneV2Page() {
  ...
  // 傻瓜模式:把每张图的"用途 + 朝向"自动拼成 @ 格式提示词(填进提示词框,用户可再改)
  // 用户已显式选好用途+朝向,故直接本地拼接,不再走后端看图自动分类。
  function composeAutoPrompt() {
    // 必选拦截:每张产品图都要选朝向(同一产品可传多角度)
    ...
    // 1. 每张图先定 @ 编号(@产品1 / @人物1 / @场景1...)
    const counts: Record<string, number> = {};
    const refOf: Record<string, string> = {};
    for (const img of images) {
      counts[img.role] = (counts[img.role] || 0) + 1;
      refOf[img.url] = `@${ROLE_LABELS[img.role]}${counts[img.role]}`;
    }

    // 2. 产品图按"用途"分组,组内把 @产品N+朝向 用、连接合成一句;人物/场景按张成句
    const prodGroups: Record<string, string[]> = {};
    const prodOrder: string[] = [];                 // 用途首次出现顺序
    const otherParts: string[] = [];
    for (const img of images) {
      const ref = refOf[img.url];
      if (img.role === "product") {
        const purpose = imagePurpose[img.url] || DEFAULT_PURPOSE.product;
        const tagged = ref + orientationText(img.url);   // @产品1正面
        if (!prodGroups[purpose]) { prodGroups[purpose] = []; prodOrder.push(purpose); }
        prodGroups[purpose].push(tagged);
      } else {
        const purpose = imagePurpose[img.url] || DEFAULT_PURPOSE[img.role] || "";
        const fn = PURPOSE_TEXT[purpose];
        if (fn) otherParts.push(fn(ref));
      }
    }
    const parts: string[] = [];
    for (const purpose of prodOrder) {
      const fn = PURPOSE_TEXT[purpose];
      if (fn) parts.push(fn(prodGroups[purpose].join("、")));   // 换成 @产品1正面、@产品2背面
    }
      ...
      <main style={{ flex: 1, padding: "2rem 2.5rem", overflowY: "auto", maxWidth: 1100, width: "100%", margin: "0 auto" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: "1rem 1.2rem", marginBottom: "1.2rem" }}>
          ...
          <div style={{ fontSize: "0.8rem", color: "#888", marginBottom: 10, lineHeight: 1.6 }}>
            为防止肖像侵权,可对上传的视频和人物图<b>自动给人脸打码/涂鸦</b>。请在上传前选择:
          </div>
