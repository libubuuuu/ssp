...
export default function VideoPage(){
  ...
  const generateReplace=async()=>{
    if(!srcVideoUrl){setError("请输入原视频链接");return;}
  ...
  const generateRemake=async()=>{
    if(!refVideoUrl){setError("请输入参考视频链接");return;}
    ...
    <div style={{display:"flex",minHeight:"100vh",background:"#edeae4",fontFamily:"-apple-system,BlinkMacSystemFont,sans-serif"}}>
      ...
      <aside style={{width:"340px",background:"#fff",borderLeft:"1px solid rgba(0,0,0,0.06)",padding:"2rem 1.75rem",display:"flex",flexDirection:"column",gap:"1.25rem",height:"100vh",position:"sticky",top:0,overflowY:"auto"}}>
        ...
        {mode==="element-replace"&&(<>
          <div>
            <div style={{fontSize:"0.72rem",color:"#999",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:"0.6rem"}}>原视频链接</div>
            <input value={srcVideoUrl} onChange={e=>setSrcVideoUrl(e.target.value)} placeholder="粘贴视频 URL..." style={{width:"100%",padding:"0.65rem 0.9rem",border:"1px solid #e5e5e5",borderRadius:"10px",fontSize:"0.85rem",background:"#fff",color:"#333",boxSizing:"border-box"}}/>
          </div>
          <div>
        ...
        {mode==="remake"&&(<>
          <div>
            <div style={{fontSize:"0.72rem",color:"#999",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:"0.6rem"}}>参考视频链接</div>
            <input value={refVideoUrl} onChange={e=>setRefVideoUrl(e.target.value)} placeholder="粘贴爆款视频 URL..." style={{width:"100%",padding:"0.65rem 0.9rem",border:"1px solid #e5e5e5",borderRadius:"10px",fontSize:"0.85rem",background:"#fff",color:"#333",boxSizing:"border-box"}}/>
