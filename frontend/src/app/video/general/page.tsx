          ...
          <>
            ...
            <div style={CARD}>
              ...
              <button onClick={analyzeVideo} disabled={loadingA || vidLoading || !refVid.url || !front.url}
                ...
                ) : scriptA ? "重新分析视频（扣24积分）" : "分析视频并生成脚本（扣24积分）"}
              </button>
                ...
                <div style={{ marginTop: "1.4rem" }}>
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: "1.2rem", flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => generateVideo(scriptA, refVid.url || undefined)} disabled={vidLoading || loadingA}
                      ...
                      {vidLoading ? <><span style={{ display: "inline-block", width: 14, height: 14, border: "2px solid #a0aec0", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />生成中…</> : (() => { const v = Math.max(65, duration * 65); const s = resolution === "1080p" ? 25 : resolution === "2k" ? 40 : 50; return `🎬 确认脚本，生成视频（视频${v} + 分辨率${s} = 扣${v + s}积分）`; })()}
                    </button>
                  </div>
                  {vidLoading && (
                        ...
                        <div>
                          ...
                          <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                            ...
                            <button onClick={() => generateVideo()} disabled={vidLoading || loading}
                              ...
                              {vidLoading ? <><span style={{ display: "inline-block", width: 12, height: 12, border: "2px solid #a0aec0", borderTopColor: "#555", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />生成中…</> : (() => { const v = Math.max(65, duration * 65); const s = resolution === "1080p" ? 25 : resolution === "2k" ? 40 : 50; return `🎬 确认，生成视频（视频${v} + 分辨率${s} = 扣${v + s}积分）`; })()}
                            </button>
                          </div>
                        </div>
                      )}
                    ...
                    <div style={{ paddingLeft: 44, marginBottom: 18 }}>
                      {!chatShowParams ? (
                        <button onClick={async () => {
                          ...
                          {(() => { const v = Math.max(65, duration * 65); const s = resolution === "1080p" ? 25 : resolution === "2k" ? 40 : 50; return `🎬 确认脚本，生成视频（视频${v} + 分辨率${s} = 扣${v + s}积分）`; })()}
                        </button>
                      ) : (
