            ...
            <div style={CARD}>
              ...
              {scriptA && !loadingA && (
                <div style={{ marginTop: "1.4rem" }}>
                  ...
                  {(() => { const v = Math.max(65, duration * 65); const tts = duration <= 5 ? 13 : duration <= 10 ? 26 : duration <= 15 ? 33 : duration <= 30 ? 52 : 104; const total = v + 26 + tts; return (<div style={{ background: "#f8f9fa", borderRadius: 10, padding: "0.8rem", marginBottom: "0.8rem", fontSize: "0.78rem", color: "#555" }}><div style={{ fontWeight: 600, marginBottom: 6 }}>📋 费用明细</div><div>🎬 视频生成：{duration}秒 × 65 = {v}积分</div><div>👤 模特头像：13积分</div><div>🖼️ 场景图：13积分</div><div>🔊 声音(TTS)：{tts}积分</div><div style={{ borderTop: "1px solid #ddd", marginTop: 6, paddingTop: 6, fontWeight: 600 }}>合计：{total}积分（¥{(total/50).toFixed(2)}）</div></div>); })()}
