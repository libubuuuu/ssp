            ...
            <div style={CARD}>
              ...
              {scriptA && !loadingA && (
                <div style={{ marginTop: "1.4rem" }}>
                  ...
                  {(() => { const v = Math.max(65, duration * 65); const tts = duration <= 5 ? 13 : duration <= 10 ? 26 : duration <= 15 ? 33 : duration <= 30 ? 52 : 104; const resPerSec = resolution === "1080p" ? 3 : resolution === "2k" ? 6 : 11; const s = resPerSec * duration; const total = v + 26 + tts + s; return (<div style={{ background: "#f8f9fa", borderRadius: 10, padding: "0.8rem", marginBottom: "0.8rem", fontSize: "0.78rem", color: "#555" }}><div style={{ fontWeight: 600, marginBottom: 6 }}>📋 费用明细</div><div>🎬 视频生成：{duration}秒 × 65 = {v}积分</div><div>👤 模特头像：13积分</div><div>🖼️ 场景图：13积分</div><div>🔊 声音(TTS)：{tts}积分</div><div>📺 分辨率({resolution})：{resPerSec} × {duration}秒 = {s}积分</div><div style={{ borderTop: "1px solid #ddd", marginTop: 6, paddingTop: 6, fontWeight: 600 }}>合计：{total}积分（¥{(total/50).toFixed(2)}）</div></div>); })()}
                  <div style={{ display: "flex", gap: 10, marginTop: "1.2rem", flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => generateVideo(scriptA, refVid.url || undefined)} disabled={vidLoading || loadingA}
                      ...
                      {vidLoading ? <><span style={{ display: "inline-block", width: 14, height: 14, border: "2px solid #a0aec0", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />生成中…</> : (() => { const v = Math.max(65, duration * 65); const tts = duration <= 5 ? 13 : duration <= 10 ? 26 : duration <= 15 ? 33 : duration <= 30 ? 52 : 104; const resPerSec = resolution === "1080p" ? 3 : resolution === "2k" ? 6 : 11; const s = resPerSec * duration; const ai = 13 + 13 + tts; return `🎬 确认脚本，生成视频（视频${v} + AI${ai} + 分辨率${s} = 扣${v + ai + s}积分）`; })()}
                    </button>
                  </div>
                  {vidLoading && (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      ...
                      {script && !loading && (
                        <div>
                          ...
                          {(() => { const v = Math.max(65, duration * 65); const resPerSec = resolution === "1080p" ? 3 : resolution === "2k" ? 6 : 11; const s = resPerSec * duration; const total = v + 102 + s; return (<div style={{ background: "#f8f9fa", borderRadius: 10, padding: "0.8rem", marginBottom: "0.8rem", fontSize: "0.78rem", color: "#555" }}><div style={{ fontWeight: 600, marginBottom: 6 }}>📋 费用明细</div><div>🎬 视频生成：{duration}秒 × 65 = {v}积分</div><div>👤 模特头像：13积分</div><div>🖼️ 场景图：13积分</div><div>🔍 趋势搜索：26积分</div><div>📝 审稿员：26积分</div><div>✍️ 文案师：13积分</div><div>🔊 声音(TTS)：11积分</div><div>📺 分辨率({resolution})：{resPerSec} × {duration}秒 = {s}积分</div><div style={{ borderTop: "1px solid #ddd", marginTop: 6, paddingTop: 6, fontWeight: 600 }}>合计：{total}积分（¥{(total/50).toFixed(2)}）</div></div>); })()}
                          <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                            ...
                            <button onClick={() => generateVideo()} disabled={vidLoading || loading}
                              ...
                              {vidLoading ? <><span style={{ display: "inline-block", width: 12, height: 12, border: "2px solid #a0aec0", borderTopColor: "#555", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />生成中…</> : (() => { const v = Math.max(65, duration * 65); const ai = 13 + 13 + 26 + 26 + 13 + 11; const s = (resolution === "1080p" ? 3 : resolution === "2k" ? 6 : 11) * duration; return `🎬 确认，生成视频（视频${v} + AI${ai} + 分辨率${s} = 扣${v + ai + s}积分）`; })()}
                            </button>
                          </div>
                        </div>
                      )}
                    ...
                    <div style={{ paddingLeft: 44, marginBottom: 18 }}>
                      {!chatShowParams ? (
                        <>
                          ...
                          {(() => { const v = Math.max(65, duration * 65); const ai = 13 + 13 + 26 + 26 + 13 + 11; const s = (resolution === "1080p" ? 3 : resolution === "2k" ? 6 : 11) * duration; return `🎬 确认脚本，生成视频（视频${v} + AI${ai} + 分辨率${s} = 扣${v + ai + s}积分）`; })()}
                        </button>
                        </>
                      ) : (
