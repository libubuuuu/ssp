...
# auto-cheap 免费优先链(全 verified 真复刻):
...
async def _run_inpainting_step(session_id: str) -> None:
    ...
    try:
        ...
        # P59 改 image_urls 用原图;P60 进一步:整段跳过 Seedream,
        # garment_url 单纯定义,reference_image = 用户原模特图,各 Step B 引擎从用户原图直读。
        garment_url: Optional[str] = (
            products[0].get("image_url") if products and products[0].get("image_url") else None
        )
            ...
            try:
                ...
                if user_prompt:
                    ...
                    qwen_instruction = (
                        ...
                        "'胸前/乳' → '前胸位置/上身'。\n\n"
                        "【精准分析原则(必读)】\n"
                        "1. 服装结构必须明确分清:**分体两件(上身 + 下身分开)** vs **连体一件(连衣裙/连体衣)**;\n"
                        "   判断依据:腰部是否有分隔线/接缝/腰带/不同颜色/不同布料;\n"
                        "   只有看到完全连贯无中断的布料才能判定连衣裙;\n"
                        "   分不清的情况下输出'分体两件(待确认)',严禁猜测。\n"
                        "2. 上衣款式必须精准:T恤(短袖+圆领)/ 长袖T / Tank top(无袖+宽肩带)/ "
                        "   细吊带(spaghetti strap)/ V领上衣 / 方领上衣 / 衬衫 / 卫衣 / 连衣裙(整身)。\n"
                        "3. 下装款式必须精准:短裤 / 长裤 / 高腰短裤 / 半身裙 / 紧身裤 / 牛仔裤。\n"
                        "4. 服装层次顺序必须明确:外层 > 中层 > 内层(最多3层),每层独立描述款式/颜色/材质。\n"
                        "5. 不确定就标'(待确认)'。**精确 > 流畅**,严禁脑补。\n\n"
                        "【画面构图】景别(大特写/特写/近景/中近景/中景/全景)、镜头角度(平视/仰视/俯视)、"
                        "镜头运动(固定/推拉/摇移/跟随)、画面比例(9:16/16:9/1:1)、画面焦点位置。\n\n"
                        "【场景背景】拍摄环境(卧室/客厅/工作室/外景等具体空间)、墙面颜色/家具/装饰物详细列举、"
                        "光线方向(顺光/侧光/逆光/顶光)、色温(暖/冷/中性)、整体氛围。\n\n"
                        "【模特特征】身材比例、肤色、发型(短/中/长 + 直/卷)、发色、"
                        "面部表情、年龄段、配饰(项链/耳环/戒指/手表),每件配饰精确描述材质和位置。\n\n"
                        "【服装层次精确分析】(P76 关键)\n"
                        "- 是否分体:明确写'分体两件(上身=X,下身=Y)' 或 '连体连衣裙' 或 '分体两件(待确认)';\n"
                        "- 外层服装:款式名 / 颜色 / 材质 / 长度;\n"
                        "- 中层(若有):款式 / 颜色 / 材质;\n"
                        "- 内层物品(被外层覆盖):款式 / 颜色 / 材质 / 出现的时间点。\n\n"
                        "【产品特征】颜色、材质、款式、关键细节(纹理/线条/支撑结构/装饰),"
                        "避免敏感词用'内层物品'统称。\n\n"
                        "【时间分镜】按每 1 秒一段精细描述,每段 120-200 字,内容必须包括:"
                        "模特身体姿态、双手位置、面部朝向、**当前帧服装层次状态(外层/中层/内层各是什么)**、"
                        "服装变化(用中性词)、镜头位置变化、画面里出现的文字(精确 OCR)。"
                        "格式严格:'0-1s:[详细描述]' / '1-2s:[详细描述]' / ...\n\n"
                        "【关键时刻】精确标注外层服装位移 / 内层物品出现的时间点(精确到 0.1s),"
                        "以及触发该变化的具体动作。\n\n"
                        "要求:每个段落充实具体,总输出 1500-2500 字。**精确 > 流畅,不确定就标'(待确认)'**。"
                    )
