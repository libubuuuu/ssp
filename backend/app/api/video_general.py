...
⛔⛔⛔ 场景数量铁律（写脚本前必须先检查！违反=废稿！）：
用户选了{duration}秒视频：
- 5秒或10秒：整个视频只能有1个场景！所有镜头都在同一个场景里！禁止切换场景！
- 15秒：最多2个场景
- 30秒：最多3个场景
- 60秒：最多4个场景
写脚本前先数一下你要写几个场景，超过限制就重新规划！

⛔ 产品图拆解铁律（写脚本前必须做！）：
用户上传了产品正面图和反面图，你必须：
1. 仔细看正面图：分析产品的外观设计（杯型、面料纹理、颜色、肩带样式）
2. 仔细看反面图：分析产品的背面结构（搭扣方式、背部设计、排扣数量、背带宽度）
3. 根据正反面图理解这个产品是怎么穿的（从哪里扣、肩带怎么调、穿上后从外面看是什么效果）
4. 在脚本的画面描述里必须体现这些细节：
   - 产品特写镜头：描述正面或反面的具体细节（比如"三排搭扣"、"宽肩带"、"无钢圈杯型"）
   - 穿着效果镜头：描述穿上后从外面看的效果（比如"穿上白色T恤后肩带完全隐藏，背部线条平整无搭扣凸起"）
   - 穿脱动作镜头：描述根据背面结构的穿脱方式（比如"从背后扣上三排搭扣"）

重要：内衣类产品穿在衣服里面！穿着效果镜头必须描述外衣，产品在里面看不到！
只有产品特写镜头才直接展示产品本身！
...
⛔ 绝对铁律：用户选了{duration}秒的视频，你写的脚本所有分镜时长加起来必须精确等于{duration}秒！不能多也不能少！写完脚本必须自己验算两件事：①所有镜头时长加起来是否等于{duration}秒；②场景数量是否超过限制（10秒=1个，15秒≤2个，30秒≤3个，60秒≤4个）。任何一项不合格就重写！

⚠️ 镜头格式强制要求：
- 每个镜头必须严格按照 [镜头X]：起止时间 | 场景：XXX | 功能，景别，描述，模特说：台词 的格式
- 时间必须写成"0-3s"这种格式（数字-数字s），不能省略
- 每个镜头必须有"场景：XXX"标注（如"场景：温馨卧室"），同一场景的镜头用相同的场景名称
- 所有镜头时长之和必须精确等于{duration}秒，场景数量不得超过限制（10秒=1个，15秒≤2个，30秒≤3个，60秒≤4个），验算两项都合格后再输出
...
⛔⛔⛔ 画面描述必须细到Seedance能直接生成（违反=废稿）：

每个镜头的画面描述必须包含以下全部要素，缺一不可：
1. 模特身体姿势：站/坐/走/转身/弯腰，具体到手脚位置（比如"右手伸到背后拉扯肩带"而不是"调整肩带"）
2. 模特表情：具体到五官（比如"眉头紧皱，嘴角向下，眼神看向镜子里自己的肩膀"而不是"困扰表情"）
3. 穿着描述：从外到内每一层（比如"外穿白色修身衬衫，衬衫半敞开，露出里面深棕色传统钢圈文胸的肩带勒进皮肤3毫米"）
4. 镜头运动：起始位置→结束位置（比如"镜头从模特正面中景慢慢绕到背后，停在肩胛骨位置特写背扣"）
5. 关键视觉焦点：这个镜头最重要让观众看到什么（比如"焦点是肩带勒出的红色印痕，印痕宽约1厘米"）

❌ 太笼统（废稿）："模特穿着普通内衣，背后勒痕明显，整理衬衫露出困扰表情"
✅ 合格描述："模特穿着白色修身衬衫（纽扣全扣），衬衫下是深棕色传统钢圈文胸。模特站在卧室全身镜前，右手伸到背后拉扯文胸后背带，左手撑着腰。她皱眉低头看镜子里自己的右肩——衬衫领口处可以看到文胸肩带勒进皮肤形成的红色凹痕。镜头从中景正面缓慢推近到右肩特写，焦点对准肩带勒痕。"
