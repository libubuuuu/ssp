...
### 现象
D-1.5 部署后用户新建任务被红条阻塞:"并行上传任务过多(5/5)"。
db `oral_sessions` 该用户只有 1 条历史 failed_step3,理论不该撞限。

### 真根因
`/api/oral/upload-chunk` L831-835 的 5/5 限制基于
`/opt/ssp/uploads/oral/_uploading/` 下文件系统目录数 — **不查 db**。
分片上传只在最后一片合并成功后才 `rmtree(upload_dir)`(L879)。

任何中途失败链路都留孤儿:
- 用户中途断网 / 关页面
- 缺片 raise(L862)
- 单片超 10MB raise(L840+)
- 累计超 200MB raise(L853)

`uploads_gc` 服务 90 天 mtime 阈值对瞬时分片完全不适用 — 一次上传
正常几秒到几十分钟,90 天等于"永不清理"。

实测该用户 5 个孤儿:1 天前 + 几小时前 D-1.5 测试期累积。每个目录里
只有 1-2 个孤立 chunk 文件(连续编号缺失),证实是中途失败。

### 修法
- **`oral.py /upload-chunk`**:5/5 检查之前加 30 分钟 mtime GC,先清孤
  儿再判配额。`time.time()` import 增加
- **`video_studio.py /upload-chunk`**:**完全一样的 bug**(L275-281),
  同样补丁应用过去
- 立刻 `rm` 该用户当前 5 个 `_uploading/{user_id}_*` 孤儿(已执行)
- 删该用户 fde80e86 死 session(failed_step3,db DELETE + 文件目录
  rm 15MB)— 任务列表 UI 干净

### 教训
**"并行任务上限"基于文件系统目录 = 中转目录,失败链路没清就永久占额。**
长上限 GC 服务跟分片中转目录的生命周期不匹配。

设计同类限制时第一个要回答的问题:
1. **数据源是 db 还是 fs?**
2. **GC 频率是否跟限制窗口匹配?**(限制 30 分钟内 5 个 → GC 至少 30 分钟跑;
   限制 90 天 N 个 → GC 90 天跑)

如果用 db 算 in_flight 状态(status IN ('uploading','asr_running',...)),
完全绕开此类问题。但 fs 桶有反 DDoS 价值(不依赖 db 一致性),取舍按场景。
